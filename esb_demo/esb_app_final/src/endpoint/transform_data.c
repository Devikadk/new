...
//if transform type is :IFSC_CODE append payload to transport_value i.e. URL
  if ((!strcmp(transform_type, ":IFSC_CODE")) && (!strcmp(transport_key, "HTTP"))) {
    ...
    //if it is  json then convert to json file (we are doing the conversion of xml to json in src/BMD/validate_request.c file, to check the size limit of payload, so we didn't do it here)
